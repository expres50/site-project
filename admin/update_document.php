...
if(isset($_POST['update_document'])){
   ...
   $update_titre =mysqli_real_escape_string($conn, $_POST['update_titre']);
   $update_matiere =mysqli_real_escape_string($conn, $_POST['update_matiere']);
   $update_niveau =mysqli_real_escape_string($conn, $_POST['update_niveau']);
   $type =mysqli_real_escape_string($conn, $_POST['type']);
   $update_pages =mysqli_real_escape_string($conn, $_POST['update_pages']);
   $update_taille =mysqli_real_escape_string($conn, $_POST['update_taille']);
   ...
   $sql=mysqli_query($conn, "UPDATE `documents` SET titre = '$update_titre', matiere = '$update_matiere', niveau = '$update_niveau', type = '$type', pages = '$update_pages', taille = '$update_taille', contenu = '$contenu' WHERE id = '$id'") or die('query failed');

     if ($sql) {
         move_uploaded_file($contenu_tmp_name, $contenu_folder);
         $message[] = "<div style='background-color: green; color: white;' class='alert success' >Le document a été modifié avec succes !</div>";
     }
     else{
        $message[] = "<div style='background-color: red; color: white;' class='alert echec'>Quelque chose s'est mal passé, ressayer !</div>";
     }
    }
...
<h2 style="text-align: center;">Modifier le document</h2>
   <?php
   ...
   <div class="flex">
      <div class="inputBox">
         <span>Titre :</span>
         <input type="text" name="update_titre" value="<?php echo $fetch['titre']; ?>" class="box form-control">
         <div>
         <span>Matière :</span>
        <select name="update_matiere" class="form-control" id="" style="width: 100%;  background-color:rgb(223, 221, 221); border: 1px solid rgb(218, 214, 214);">
         ...
         <div>
         <span>Niveau :</span>
            <select name="update_niveau" class="form-control" id="" style="width: 100%; background-color:rgb(223, 221, 221); border: 1px solid rgb(218, 214, 214);">
                <option value="" autofocus>Choisir...</option>
         ...
         <span>Type</span>
         <div>
               <select name="type" class="form-control" id="" style="width: 100%; background-color:rgb(223, 221, 221); border: 1px solid rgb(218, 214, 214);">
                  <option value="" autofocus>Choisir...</option>
      ...
      <div class="inputBox">
         <span>Pages :</span>
         <input type="number" name="update_pages" value="<?php echo $fetch['pages']; ?>" class="box form-control">
         <span>Taille</span>
         <input type="text" name="update_taille" value="<?php echo $fetch['taille']; ?>"  class="box form-control">
         <span>Modifier le fichier :</span>
         <input type="file" name="contenu" accept="application/pdf" class="box">
      </div>
   </div>
   <input type="submit" value="Mettre à jour" name="update_document" class="btn">
   <a href="documents.php?niveau=tous" class="delete-btn">Retour aux documents</a>
</form>
